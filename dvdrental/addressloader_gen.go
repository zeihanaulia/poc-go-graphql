...
type AddressLoader struct {
	...
	fetch func(keys []int) ([]*models.Address, []error)
...
func (l *AddressLoader) Load(key int) (*models.Address, error) {
...
func (l *AddressLoader) LoadThunk(key int) func() (*models.Address, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*models.Address, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *AddressLoader) LoadAll(keys []int) ([]*models.Address, []error) {
	results := make([]func() (*models.Address, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	addresss := make([]*models.Address, len(keys))
...
func (l *AddressLoader) Prime(key int, value *models.Address) bool {
	l.mu.Lock()
	var found bool
...
func (l *AddressLoader) unsafeSet(key int, value *models.Address) {
	if l.cache == nil {
		l.cache = map[int]*models.Address{}
	}
	l.cache[key] = value
}
